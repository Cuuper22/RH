...
Substrate: Zeta23/PrimeSideA/EndsWeighted.lean ; 1-D estimates N1/N2:
...
theorem majK2_integrable (hνc : Continuous ν) (hF : LocalHypsCoreW cϱ p F) (hν : NuBound p B ν)
    (hB0 : 0 ≤ B) (hT : 2 * π ≤ p.T) : Integrable (majK2 cϱ p ν) := by
  ...
  have h : Integrable (fun q : ℝ × ℝ => ∑ k ∈ Finset.range p.d,
    ...
    refine integrable_finset_sum _ fun k hk => ?_
...
for T ≥ T₀ — the ν-dependence sits in a nonnegative bilinear kernel (Cauchy–Schwarz in χ
can be pushed inside this). -/
theorem calE2_maj_bound :
  ...
  refine ⟨2 * (12 + 4 * max 0 (Real.log cϱ) + 4 * cϱ) * CN2 cϱ,
    2 * π * Real.exp 8, fun p F B ν hplam hT hF hL2l hνc hν hBl => ?_⟩
  -- regime
  ...
  have hsum_int : Integrable (fun τ : ℝ =>
    ...
    have := integrable_finset_sum (Finset.range p.d) hwk
  ...
  have hB : ∫ q in (sqI p)ᶜ, majK2 cϱ p ν q
    ...
    have hmono : ∫ q in S₂ \ S₁, majK2 cϱ p ν q ≤ ∫ q in S₂, majK2 cϱ p ν q :=
      setIntegral_mono_set habs.integrableOn
        (Filter.Eventually.of_forall fun q => hMnn q)
        ((Set.diff_subset : S₂ \ S₁ ⊆ S₂).eventuallyLE)
    linarith
  ...
  have hmaj_int : ∀ τ : ℝ, Integrable (fun τ' : ℝ =>
    ...
    fun τ => integrable_finset_sum _ (fun k hk => (hwk k hk).const_mul _)
  have hinner1 : ∀ τ : ℝ, (∫ τ', majK2 cϱ p ν (τ, τ'))
    ...
    refine h1.trans ?_
    rw [integral_finset_sum _ (fun k hk => (hwk k hk).const_mul _)]
  ...
  -- Step D: iterated-integral bound over S₂ (constrained second coordinate)
  have hrestrict2 : (volume : Measure (ℝ × ℝ)).restrict S₂
      = (volume : Measure ℝ).prod (volume.restrict ((p.I)ᶜ)) := by
    rw [hS₂, Measure.volume_eq_prod, ← Measure.prod_restrict, Measure.restrict_univ]
  have hint2 : Integrable (fun q : ℝ × ℝ => majK2 cϱ p ν q)
      ((volume : Measure ℝ).prod (volume.restrict ((p.I)ᶜ))) := by
    rw [← hrestrict2]; exact habs.integrableOn
  have hiter2 : ∫ q in S₂, majK2 cϱ p ν q
      = ∫ τ, ∫ τ' in (p.I)ᶜ, majK2 cϱ p ν (τ, τ') := by
    rw [show (∫ q in S₂, majK2 cϱ p ν q)
        = ∫ q, majK2 cϱ p ν q ∂((volume : Measure (ℝ × ℝ)).restrict S₂) from rfl,
      hrestrict2, integral_prod _ hint2]
  have hg_nn : ∀ k : ℕ, 0 ≤ ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'| :=
    fun k => setIntegral_nonneg hmI fun τ' _ => mul_nonneg (hψnn τ' k) (abs_nonneg _)
  have hinner2 : ∀ τ : ℝ, (∫ τ' in (p.I)ᶜ, majK2 cϱ p ν (τ, τ'))
      ≤ ∑ k ∈ Finset.range p.d, (p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
          * ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'| := by
    intro τ
    have h1 : (∫ τ' in (p.I)ᶜ, majK2 cϱ p ν (τ, τ')) ≤ ∫ τ' in (p.I)ᶜ,
        ∑ k ∈ Finset.range p.d, (p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
          * (psiA cϱ p (τ' - p.tau k) * |ν τ'|) :=
      integral_mono_of_nonneg (Filter.Eventually.of_forall fun τ' => hMnn (τ, τ'))
        (hmaj_int τ).integrableOn (Filter.Eventually.of_forall fun τ' => hprod_bound τ τ')
    refine h1.trans (le_of_eq ?_)
    rw [integral_finset_sum _ (fun k hk => ((hwk k hk).const_mul _).integrableOn)]
    refine Finset.sum_congr rfl fun k hk => ?_
    rw [integral_const_mul]
  have houter2 : ∫ q in S₂, majK2 cϱ p ν q
      ≤ p.L ^ 2 * M1 * (CN2 cϱ * (B * (p.L * p.l))) := by
    rw [hiter2]
    have hdom_int : Integrable (fun τ : ℝ => ∑ k ∈ Finset.range p.d,
        (p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
          * ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'|) := by
      refine integrable_finset_sum _ (fun k hk => ?_)
      have := ((hwk k hk).const_mul (p.L ^ 2)).mul_const
        (∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'|)
      refine this.congr (Filter.Eventually.of_forall fun τ => ?_)
      ring
    have h2 : ∫ τ, (∫ τ' in (p.I)ᶜ, majK2 cϱ p ν (τ, τ'))
        ≤ ∫ τ, ∑ k ∈ Finset.range p.d,
            (p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
              * ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'| :=
      integral_mono_of_nonneg
        (Filter.Eventually.of_forall fun τ => setIntegral_nonneg hmI fun τ' _ => hMnn (τ, τ'))
        hdom_int (Filter.Eventually.of_forall fun τ => hinner2 τ)
    refine h2.trans ?_
    rw [integral_finset_sum _ (fun k hk => by
      have := ((hwk k hk).const_mul (p.L ^ 2)).mul_const
        (∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'|)
      exact this.congr (Filter.Eventually.of_forall fun τ => by ring))]
    have h3 : ∀ k ∈ Finset.range p.d,
        (∫ τ, (p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
          * ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'|)
        ≤ (p.L ^ 2 * M1) * ∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'| := by
      intro k hk
      rw [integral_mul_const]
      refine mul_le_mul_of_nonneg_right ?_ (hg_nn k)
      have h4 : (∫ τ, p.L ^ 2 * (psiA cϱ p (τ - p.tau k) * |ν τ|))
          = p.L ^ 2 * ∫ τ, psiA cϱ p (τ - p.tau k) * |ν τ| :=
        integral_const_mul _ _
      have h5 : (∫ τ, p.L ^ 2 * psiA cϱ p (τ - p.tau k) * |ν τ|)
          = p.L ^ 2 * ∫ τ, psiA cϱ p (τ - p.tau k) * |ν τ| := by
        rw [← h4]
        congr 1
        funext τ
        ring
      rw [h5]
      exact mul_le_mul_of_nonneg_left (hN1 _ (htau k hk)) (by positivity)
    refine (Finset.sum_le_sum h3).trans ?_
    rw [← Finset.mul_sum]
    have h6 : ∑ k ∈ Finset.range p.d,
        (∫ τ' in (p.I)ᶜ, psiA cϱ p (τ' - p.tau k) * |ν τ'|)
        = ∫ τ' in (p.I)ᶜ, (∑ k ∈ Finset.range p.d, psiA cϱ p (τ' - p.tau k)) * |ν τ'| := by
      rw [← integral_finset_sum _ (fun k hk => (hwk k hk).integrableOn)]
      congr 1
      funext τ'
      rw [Finset.sum_mul]
    rw [h6]
...
theorem calE2_maj_bound_L :
  ...
  have hinner1 : ∀ τ : ℝ, (∫ τ', majK2 cϱ p ν (τ, τ'))
    ...
    ring
  have houter1 : ∫ q in S₁, majK2 cϱ p ν q
      ≤ p.L ^ 2 * M1 * (CN2 cϱ * (B * (p.L * (p.L + p.l)))) := by
    rw [hiter1]
    have h1 : ∫ τ in (p.I)ᶜ, (∫ τ', majK2 cϱ p ν (τ, τ'))
        ≤ ∫ τ in (p.I)ᶜ, p.L ^ 2 * M1
            * ((∑ k ∈ Finset.range p.d, psiA cϱ p (τ - p.tau k)) * |ν τ|) :=
