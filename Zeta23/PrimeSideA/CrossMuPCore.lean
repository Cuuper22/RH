...
This file depends only on `Zeta23.PrimeSideA.Defs` (the definition of `Mform`).
-/
...
/-- (local copy of `PrimeSide.Mform_integrableOn`, to keep this file's imports minimal) -/
...
/-- `∫_S Φ(x−y)² dy ≤ ∫_ℝ Φ²` (local copy of `PrimeSide.setIntegral_sq_shift_le`). -/
...
/-- **IBP in `τ′`** (§5.4 "integrating by parts"), for fixed `τ` and `y ≠ 0`:
`∫_T^{2T} Φ(τ−τ′)² cos(τ′y) dτ′ = [Φ(τ−τ′)² sin(τ′y)/y]_T^{2T} + y⁻¹ ∫_T^{2T} (Φ²)′(τ−τ′) sin(τ′y) dτ′`. -/
lemma inner_ibp (hΦ : ContDiff ℝ 1 Φ) {y : ℝ} (hy : y ≠ 0) (τ : ℝ) :
    ∫ τ' in T..(2 * T), Φ (τ - τ') ^ 2 * Real.cos (τ' * y)
      = (Φ (τ - 2 * T) ^ 2 * Real.sin (2 * T * y) - Φ (τ - T) ^ 2 * Real.sin (T * y)) / y
        + y⁻¹ * ∫ τ' in T..(2 * T), deriv (fun x => Φ x ^ 2) (τ - τ') * Real.sin (τ' * y) := by
  ...
  have hu : ∀ x ∈ uIcc T (2 * T), HasDerivAt (fun τ' => Ψ (τ - τ')) (-deriv Ψ (τ - x)) x := by
    intro x _
    have h := (hΨd (τ - x)).comp x ((hasDerivAt_id x).const_sub τ)
    simpa [Function.comp_def] using h
  have hv : ∀ x ∈ uIcc T (2 * T),
      HasDerivAt (fun τ' => Real.sin (τ' * y) / y) (Real.cos (x * y)) x := by
    intro x _
    have h := (((hasDerivAt_id x).mul_const y).sin).div_const y
  ...
  have hibp := intervalIntegral.integral_mul_deriv_eq_deriv_mul hu hv
    ...
    ((Real.continuous_cos.comp (continuous_id.mul continuous_const)).intervalIntegrable _ _)
  -- hibp : ∫ Ψ(τ-τ') cos(τ'y) = Ψ(τ-2T) sin(2Ty)/y - Ψ(τ-T) sin(Ty)/y - ∫ (-Ψ'(τ-τ')) (sin(τ'y)/y)
  simp only [hΨdef] at hibp ⊢
  rw [hibp]
  have e : ∫ x in T..(2 * T), -deriv (fun x => Φ x ^ 2) (τ - x) * (Real.sin (x * y) / y)
      = -(y⁻¹ * ∫ x in T..(2 * T), deriv (fun x => Φ x ^ 2) (τ - x) * Real.sin (x * y)) := by
...
theorem abs_Mform_cos_le (hT : 0 ≤ T) (hΦ : ContDiff ℝ 1 Φ)
  ...
  -- the pieces
  set A : ℝ → ℝ := fun τ => Ψ (τ - 2 * T) * Real.sin (2 * T * y) - Ψ (τ - T) * Real.sin (T * y)
    with hAdef
  set F : ℝ → ℝ → ℝ := fun τ τ' => u τ * ψ (τ - τ') * Real.sin (τ' * y) with hFdef
  have hFc : Continuous (Function.uncurry F) := by
    show Continuous (fun q : ℝ × ℝ => u q.1 * ψ (q.1 - q.2) * Real.sin (q.2 * y))
    fun_prop
  ...
  have step1 : Mform Φ T u (fun t => Real.cos (t * y))
      = y⁻¹ * ((∫ τ in Icc T (2 * T), u τ * A τ) + ∫ τ in Icc T (2 * T), ∫ τ' in Icc T (2 * T), F τ τ') := by
    rw [Mform_eq_iter hΦc huc (by fun_prop)]
    have hinner : ∀ τ, ∫ τ' in Icc T (2 * T), Φ (τ - τ') ^ 2 * Real.cos (τ' * y)
        = A τ / y + y⁻¹ * ∫ τ' in Icc T (2 * T), ψ (τ - τ') * Real.sin (τ' * y) := by
      intro τ
    ...
    have e : ∀ τ, u τ * (A τ / y + y⁻¹ * ∫ τ' in Icc T (2 * T), ψ (τ - τ') * Real.sin (τ' * y))
        = y⁻¹ * (u τ * A τ) + y⁻¹ * ∫ τ' in Icc T (2 * T), F τ τ' := by
      intro τ
      have hF' : ∫ τ' in Icc T (2 * T), F τ τ'
          = u τ * ∫ τ' in Icc T (2 * T), ψ (τ - τ') * Real.sin (τ' * y) := by
        rw [← integral_const_mul]
  ...
  have bd1 : |∫ τ in Icc T (2 * T), u τ * A τ| ≤ 2 * B * S := by
    ...
    · refine ae_restrict_of_forall_mem hIm fun τ hτ => ?_
      ...
      nlinarith [Real.abs_sin_le_one (2 * T * y), Real.abs_sin_le_one (T * y), hΨnn (τ - 2 * T),
        hΨnn (τ - T), abs_nonneg (Real.sin (2 * T * y)), abs_nonneg (Real.sin (T * y))]
    · have i1 : IntegrableOn (fun τ => Ψ (τ - 2 * T)) (Icc T (2 * T)) := by
      ...
      nlinarith
  have bd2 : |∫ τ' in Icc T (2 * T), Real.sin (τ' * y) * (u (2 * T) * Ψ (2 * T - τ') - u T * Ψ (T - τ')
    ...
    · refine ae_restrict_of_forall_mem hIm fun τ' hτ' => ?_
      rw [Real.norm_eq_abs, abs_mul]
      have hsin := Real.abs_sin_le_one (τ' * y)
      ...
      calc |Real.sin (τ' * y)| * |u (2 * T) * Ψ (2 * T - τ') - u T * Ψ (T - τ')
...
/-- **IBP in `τ′`, phase-shifted version** (as `inner_ibp`, for `cos(τ′y + θ)`). -/
...
/-- **The oscillatory bound, phase-shifted version**: same hypotheses and bound as
`abs_Mform_cos_le`, for `v(t) = cos(t·y + θ)` (covers `sin` at `θ = −π/2`; used for the
complex-coefficient prime sums of Theorem E, where `Re(c·e^{−iτy}) = ‖c‖·cos(τy − arg c)`). -/
