...
-/
/-
Zeta23/ThmE/IntMuChi.lean — the [eq:muints]-type integrals for μ_χ.

Route: the proofs in Zeta23/GammaFacts/IntMu.lean are generic in the function — they are re-run
here for an arbitrary continuous g with |g(τ) − (1/2π)log(|τ|/2π)| ≤ C/τ² (section Generic,
with Zeta23.mu → g; the FTC anchors MuInts.integral_main_eq / integral_main_sq_eq are reused).
Then μ_χ = g + (log q)/(2π) with g := μ_χ − (log q)/(2π), since the conductor cancels in the
Stirling field:
...
theorem int_muq_of_stirling (hq : 1 ≤ q) (hcont : Continuous (muq κ q))
  ...
  have hgst : ∃ C : ℝ, ∀ τ : ℝ, 1 ≤ |τ| →
    ...
    rw [e]
    exact hC τ hτ
  obtain ⟨C, T₀, h⟩ := int_g_of_stirling hgcont hgst
  refine ⟨C, max T₀ 1, fun T hT => ?_⟩
...
theorem int_muq_sq_of_stirling (hq : 1 ≤ q) (hcont : Continuous (muq κ q))
  ...
  have hgst : ∃ C : ℝ, ∀ τ : ℝ, 1 ≤ |τ| →
    ...
    rw [e]
    exact hC τ hτ
  obtain ⟨C₁, T₁, h₁⟩ := int_g_sq_of_stirling hgcont hgst
  obtain ⟨C₂, T₂, h₂⟩ := int_g_of_stirling hgcont hgst
  -- thresholds: T ≥ T₁, T₂, and large enough that ell1 T ≥ 1, l T ≥ 1, T ≥ 1
  refine ⟨|C₁| + 2 * c₀ * |C₂| * (4 * Real.pi ^ 2),
    max (max T₁ T₂) (2 * Real.pi * Real.exp 2), fun T hT => ?_⟩
  ...
  have hTe : 2 * Real.pi * Real.exp 2 ≤ T := le_trans (le_max_right _ _) hT
  have hexp1 : (1 : ℝ) ≤ Real.exp 2 := by have := Real.add_one_le_exp (2 : ℝ); linarith
  have hT0 : (0 : ℝ) < T := by nlinarith [Real.exp_pos (2 : ℝ), Real.pi_gt_three]
  have hT1 : (1 : ℝ) ≤ T := by nlinarith [Real.pi_gt_three]
  have hl2 : (2 : ℝ) ≤ l T := by
    have h1 : Real.exp 2 ≤ T / (2 * Real.pi) := by
      rw [le_div_iff₀ (by positivity)]; linarith
    have h2 := Real.log_le_log (Real.exp_pos 2) h1
  ...
  have hlT : l T ≤ T := by
    ...
    linarith
  have hkey : 1 / T ≤ (4 * Real.pi ^ 2) * Xq / l T ^ 2 := by
    rw [hXq, div_le_div_iff₀ hT0 hl2']
    have e : (4 * Real.pi ^ 2) * (T * ell1q q T ^ 2 / (4 * Real.pi ^ 2)) * T = T ^ 2 * ell1q q T ^ 2 := by
      field_simp
    rw [one_mul, e]
    have h3 : l T ^ 2 ≤ T ^ 2 := by nlinarith
    have h4 : T ^ 2 ≤ T ^ 2 * ell1q q T ^ 2 :=
      le_mul_of_one_le_right (by positivity) (one_le_pow₀ hellq1)
    linarith
  -- the two generic estimates
  have E₁ := h₁ T hT₁
  have E₂ := h₂ T hT₂
  -- decomposition ∫μ² = ∫g² + 2c₀∫g + c₀²T
  have hgi : IntervalIntegrable g volume T (2 * T) := hgcont.intervalIntegrable _ _
  ...
  have ediff : (∫ τ in T..(2 * T), g τ ^ 2) + 2 * c₀ * (∫ τ in T..(2 * T), g τ) + c₀ ^ 2 * T - Xq
    ...
    rw [htarget]; ring
  rw [ediff]
  have b1 : |(∫ τ in T..(2 * T), g τ ^ 2) - X| ≤ |C₁| * Xq / l T ^ 2 := by
    calc |(∫ τ in T..(2 * T), g τ ^ 2) - X|
      ...
      _ ≤ |C₁| * Xq / l T ^ 2 := by
          apply div_le_div_of_nonneg_right _ hl2'.le
          exact mul_le_mul_of_nonneg_left hXle (abs_nonneg _)
  have b2 : |2 * c₀ * ((∫ τ in T..(2 * T), g τ) - T * ell1 T / (2 * Real.pi))|
      ≤ 2 * c₀ * |C₂| * (4 * Real.pi ^ 2) * Xq / l T ^ 2 := by
    rw [abs_mul, abs_of_nonneg (by positivity : (0:ℝ) ≤ 2 * c₀)]
    have h4 : |(∫ τ in T..(2 * T), g τ) - T * ell1 T / (2 * Real.pi)| ≤ |C₂| / T :=
      le_trans E₂ (div_le_div_of_nonneg_right (le_abs_self _) hT0.le)
    have h5 : |C₂| / T ≤ |C₂| * ((4 * Real.pi ^ 2) * Xq / l T ^ 2) := by
      rw [div_eq_mul_one_div]
      exact mul_le_mul_of_nonneg_left hkey (abs_nonneg _)
    calc 2 * c₀ * |(∫ τ in T..(2 * T), g τ) - T * ell1 T / (2 * Real.pi)|
        ≤ 2 * c₀ * (|C₂| * ((4 * Real.pi ^ 2) * Xq / l T ^ 2)) :=
          mul_le_mul_of_nonneg_left (le_trans h4 h5) (by positivity)
      _ = 2 * c₀ * |C₂| * (4 * Real.pi ^ 2) * Xq / l T ^ 2 := by ring
  calc |((∫ τ in T..(2 * T), g τ ^ 2) - X)
        + 2 * c₀ * ((∫ τ in T..(2 * T), g τ) - T * ell1 T / (2 * Real.pi))|
      ≤ |(∫ τ in T..(2 * T), g τ ^ 2) - X|
        + |2 * c₀ * ((∫ τ in T..(2 * T), g τ) - T * ell1 T / (2 * Real.pi))| := abs_add_le _ _
    _ ≤ |C₁| * Xq / l T ^ 2 + 2 * c₀ * |C₂| * (4 * Real.pi ^ 2) * Xq / l T ^ 2 := add_le_add b1 b2
    _ = (|C₁| + 2 * c₀ * |C₂| * (4 * Real.pi ^ 2)) * Xq / l T ^ 2 := by ring
