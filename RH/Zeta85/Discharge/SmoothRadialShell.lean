...
open Filter Matrix Finset Set MeasureTheory
...
theorem shrinkingHalfPeriodBump_rOut_lt
  ...
  have hn : 0 < (n : ℝ) + 3 := by positivity
  have hq : 0 < L / (2 * ((n : ℝ) + 3)) :=
    div_pos hL (mul_pos (by norm_num) hn)
  linarith
...
theorem tendsto_shrinkingProfileWindow_sq
  ...
  · have habsScaled : |u / L| ≤ (1 : ℝ) / 2 := by
    ...
    have hmem :
      ...
      constructor
      · linarith [(abs_le.mp habsScaled).1]
      · exact (abs_le.mp habsScaled).2
    have htarget :
...
theorem integral_supportedFullProfile
  ...
  calc
    (∫ x : ℝ, @QuarticGramFamily.supportedFullProfile v x) =
        ∫ x : ℝ,
          (Icc (-(1 : ℝ) / 2) (1 / 2)).indicator v x := by
      apply MeasureTheory.integral_congr_ae
      filter_upwards [] with x
      rw [QuarticGramFamily.supportedFullProfile]
    _ = ∫ x in (-(1 : ℝ) / 2)..(1 / 2), v x := by
  ...
  · by_cases hmem :
    ...
    · have habs : |u / L| ≤ (1 : ℝ) / 2 := by
      ...
      have hprod :
        ...
        mul_nonneg hb0 (sub_nonneg.mpr hb1)
      have hbsq :
          (shrinkingHalfPeriodBump L n hL) u ^ 2 ≤ 1 := by
        nlinarith [hprod]
      simpa only [mul_one] using
        mul_le_mul_of_nonneg_left hbsq hvnonneg
...
theorem tendsto_integral_shrinkingProfileShellWindow_sq_mul_cexp
  ...
  · filter_upwards [
    ...
    have hcast :
      ...
      simpa only [Function.comp_def, Complex.ofReal_pow] using
...
theorem tendsto_shrinkingProfileShellNormalizedPairKernel
  ...
  have hmassC :
    ...
    simpa only [Function.comp_def,
      ← integral_complex_ofReal, Complex.ofReal_pow] using
