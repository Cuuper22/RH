...
/-- A complete physical family with a central half-period core and outer
...
structure CoreData
  ...
  supportRadius :
    ∀ T : ℝ, Fin (F.channelCount T) → ℝ
  core :
    ∀ T : ℝ, Finset (Fin (F.channelCount T))
  shell :
    ∀ T : ℝ, Fin (F.channelCount T) → ℕ
  innerRadius :
    ∀ T : ℝ, Fin (F.channelCount T) → ℝ
  ...
  core_support :
    ∀ T j, j ∈ core T →
      tsupport (F.window T j) ⊆
        Icc (-commonPeriod T / 2) (commonPeriod T / 2)
  shell_support :
    ∀ T j, j ∉ core T → ∀ u, F.window T j u ≠ 0 →
      innerRadius T j < |u| ∧ |u| < outerRadius T j
  shell_inner :
    ∀ T j, j ∉ core T →
      (shell T j : ℝ) * commonPeriod T / 2 <
...
def CoreData.toCollectiveWindowRegularity
  ...
  refine
    { commonPeriod := h.commonPeriod
      ...
      aggregate_aliases_cancel := ?_ }
  intro T
  exact RadialShellAlias.aggregateAliasCancellation_of_coreAndRadialShells
    T (h.commonPeriod T) (h.supportRadius T) (F.window T)
    (h.core T) (h.shell T) (h.innerRadius T) (h.outerRadius T)
    (h.period_pos T) (h.supportRadius_nonneg T)
    (h.smooth T) (h.support T) (h.even T)
    (h.core_support T) (h.shell_support T)
    (h.shell_inner T) (h.shell_outer T)

/-- A terminal literal energy-tail estimate for a core-plus-shell family
hands directly to the isometric quartic lower bound. -/
theorem CoreData.literalEnergyTailLowerBound_toIsometric
    {Z : ZeroConfig} {σ μ p : ℝ} {v : ℝ → ℝ}
    {F : QuarticGramFamily Z σ μ p v}
    {q : TrimmedMoment.Quartic}
    (hshell : CoreData F)
